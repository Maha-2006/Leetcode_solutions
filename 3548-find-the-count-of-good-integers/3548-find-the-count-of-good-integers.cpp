class Solution {
    ...
    void precomputeFactorial(int n) {
        fact.resize(n + 1);  // Ensure that the vector is resized to hold n + 1 elements
        fact[0] = 1;
        fact[1] = 1;
        for (ll i = 2; i <= n; ++i) {  // Change 10 to n to compute factorial up to n
            fact[i] = i * fact[i - 1];
        }
    }
...
public:
    long long countGoodIntegers(int n, int k) {
        fact.clear();  // Clear any previous data in the fact vector
        fact.resize(n + 1);  // Correctly resize the fact vector to hold n + 1 elements
